...
def _load_sql_script() -> str:
    if os.path.isfile(LOCAL_SQL_PATH):
        logger.info(f"Loading subscription billing SQL from local file: {LOCAL_SQL_PATH}")
        with open(LOCAL_SQL_PATH, "r", encoding="utf-8") as f:
            return f.read()
    raise FileNotFoundError(f"Required database seed file not found: {LOCAL_SQL_PATH}")
...
def _create_engine():
    ...
    logger.info("Subscription billing database loaded successfully into memory.")
